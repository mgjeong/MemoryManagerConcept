...
type InternalContainerLifecycle interface {
	PreStartContainer(pod *v1.Pod, container *v1.Container, containerID string, podCgroupPath string) error
...
func (i *internalContainerLifecycleImpl) PreStartContainer(pod *v1.Pod, container *v1.Container, containerID string, podCgroupPath string) error {
	...
	if utilfeature.DefaultFeatureGate.Enabled(kubefeatures.TopologyManager) {
		...
		//ToDo: we should decide this statements move outside or not.
		if utilfeature.DefaultFeatureGate.Enabled(kubefeatures.MemoryManager) {
			err := i.memoryManager.AddContainer(pod, container, containerID, podCgroupPath)
			if err != nil {
				return err
			}
		}
