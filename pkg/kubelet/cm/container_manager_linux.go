...
func NewContainerManager(mountUtil mount.Interface, cadvisorInterface cadvisor.Interface, nodeConfig NodeConfig, failSwapOn bool, devicePluginEnabled bool, recorder record.EventRecorder) (ContainerManager, error) {
	...
	if utilfeature.DefaultFeatureGate.Enabled(kubefeatures.MemoryManager) {
		...
		klog.Infof("[memorymanager] Initilizing Memory Manager with %s policy", nodeConfig.ExperimentalMemoryManagerPolicy)
		if err != nil {
...
func (cm *containerManagerImpl) InternalContainerLifecycle() InternalContainerLifecycle {
	return &internalContainerLifecycleImpl{cm.cpuManager, cm.topologyManager, cm.memoryManager}
}
...
func (cm *containerManagerImpl) Start(node *v1.Node,
	...
	// Initialize memory manager
	if utilfeature.DefaultFeatureGate.Enabled(kubefeatures.MemoryManager) {
		cm.memoryManager.Start(memorymanager.ActivePodsFunc(activePods), podStatusProvider, runtimeService)
	}
