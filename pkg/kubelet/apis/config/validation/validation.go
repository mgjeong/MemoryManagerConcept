...
func ValidateKubeletConfiguration(kc *kubeletconfig.KubeletConfiguration) error {
	...
	if kc.MemoryManagerPolicy == "singleNUMA" && !localFeatureGate.Enabled(features.MemoryManager) {
		allErrors = append(allErrors, fmt.Errorf("invalid configuration: MemoryManager %v requires feature gate MemoryManager", kc.MemoryManagerPolicy))
	}
